...
# def read_lines_and_process(file_path):

     #build a dictionary associating each species with reactions where it is a product and a reactant
...
def update_species_rxn_dict(rxn_name, species_rxn_dict):
    
    reactant_string, product_string = rxn_name.split(" => ")[0], rxn_name.split(" => ")[1]
    reactant_list = reactant_string.split(' + ')
    product_list = product_string.split(' + ')
    
    for reactant in reactant_list:
        species_rxn_dict.update({reactant:{"reactant":rxn_name}})
        
    for product in product_list:
        species_rxn_dict.update({product:{"product":rxn_name}})
os.chdir("G:\My Drive\Kinetiscope\Allabsorbers_020524") 
file_path = "rxn_list.txt"

with open(file_path) as f:
    file_list = f.readlines()

index_dict = {} #read in names of reactions from kinetiscope, associating them with their selection frequencies
species_rxn_dict = {}
index = 1

for line in file_list:
    if "Equation" in line:
        rxn_name = line.split(": ")[1].strip('\n')
        index_dict[index] = {"rxn_name":rxn_name}
        index += 1
        
        update_species_rxn_dict(rxn_name, species_rxn_dict)
        
        
            
        print(species_rxn_dict)
        sys.exit()
...
#input the name of the product for which we want to do pathfinding
