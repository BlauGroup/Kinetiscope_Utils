...
from classify_chemical_reactions import (
    tag_chemical_reaction,
    process_chemical_reactions
)
from monty.serialization import loadfn, dumpfn
import copy
import operator
...
P1_rxn_tally = loadfn("reaction_tally.json") #loads json as a dictionary whose keys are mol ids and values are
...
for rxn_dict in all_P1_reactions:
    ...
    if rxn_is_ionization(new_rxn):
    
        rxns_for_simulation, rxns_already_added = \
        process_ionization_reactions(new_rxn, rxns_for_simulation, rxns_already_added)
    
    else:
        
        rxns_for_simulation, rxns_already_added = \
        process_chemical_reactions(new_rxn, rxns_for_simulation, rxns_already_added)
